...
def signup_user_handler(body: UserSignRequest, background_tasks: BackgroundTasks, session = Depends(get_session)):
    # 이메일 중복 검사
    ...
    background_tasks.add_task(send_welcome_email, email=body.email)
...
def login_user_handler(body: UserLoginRequest, session = Depends(get_session)):
    #사용자 조회
    # with SessionFactory() as session:
    stmt = select(User).where(User.email == body.email)
    user = session.scalar(stmt)
