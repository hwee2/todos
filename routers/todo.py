...
def get_todos_handler(authorization: HTTPAuthorizationCredentials | None = Depends(bearer)):
    user_id = None
    if authorization:
...
def get_todo_handler(todo_id: int, authorization: HTTPAuthorizationCredentials | None = Depends(bearer)):
    user_id = None
    if authorization:
        token = authorization.credentials
        user_id = decode_access_token(token)
...
def create_todo_handler(body: TodoCreateRequest, authorization: HTTPAuthorizationCredentials | None = Depends(bearer)): # 요청 본문 매개변수로 받기
    user_id = None
    if authorization:
        token = authorization.credentials
        user_id = decode_access_token(token)
...
def update_todo_handler(todo_id: int, body: TodoUpdateRequest, authorization: HTTPAuthorizationCredentials | None = Depends(bearer)):
    user_id = None
    if authorization:
        token = authorization.credentials
        user_id = decode_access_token(token)
...
def delete_todo_handler(todo_id: int, authorization: HTTPAuthorizationCredentials | None = Depends(bearer)):
    user_id = None
    if authorization:
        token = authorization.credentials
        user_id = decode_access_token(token)
